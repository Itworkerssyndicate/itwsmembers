...
(function () {
  ...
  /* ============================================
     COPY TO CLIPBOARD
     ============================================ */
  async function copyToClipboard(text, label = 'النص') {
    if (!text) {
    ...
    try {
      ...
      showToast(`تم نسخ ${label}`, 'success', 1500);
    } catch (e) {
  ...
  const ICONS = {
    ...
    mail: '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>',
  ...
  async function checkAuth() {
    try {
      ...
      if (!allowedRoles.includes(userRole)) {
        alert('هذه الصفحة مخصصة للعلاقات العامة فقط');
      ...
      console.error('[PublicRelations] Auth error:', err);
      window.location.href = 'login.html';
      return false;
    }
  }
  ...
  async function loadLookups() {
    try {
      ...
      console.error('[PublicRelations] Lookups error:', err);
    }
  }
  ...
  async function loadStats() {
    try {
      const { data, error } = await client
        .from('members')
        .select('phone, email, governorate')
        .eq('is_active', true);
      ...
      data.forEach(m => {
        if (m.phone) withPhone++;
        if (m.email) withEmail++;
      ...
      set('statWithPhone', withPhone);
      set('statWithEmail', withEmail);

    } catch (e) {
      console.error('[PublicRelations] Stats error:', e);
    }
  }
  ...
  async function loadReport() {
    ...
    try {
      ...
      console.error('[PublicRelations] Report error:', e);
    }
  }
  ...
  async function loadMembers() {
    ...
    try {
      ...
      if (filters.search) {
        ...
        query = query.or(
          `full_name.ilike.%${s}%,phone.ilike.%${s}%,email.ilike.%${s}%,national_id.ilike.%${s}%,membership_no.ilike.%${s}%`
        );
      }
      ...
      console.error('[PublicRelations] Load error:', err);
      if (tbody) {
        tbody.innerHTML = `
          <tr>
  ...
  function renderTable() {
            ...
            <div class="contact-cell">
              ${m.phone ? `
                <div style="display:flex;align-items:center;gap:6px;">
                  ...
                  <button type="button" class="copy-contact-btn" data-copy="${escapeHtml(m.phone)}" data-label="الموبايل" title="نسخ">
                    ${ICONS.copy}
                  </button>
                </div>
              ` : ''}
              ${m.email ? `
                <div style="display:flex;align-items:center;gap:6px;">
                  <div class="contact-row">
                    ${ICONS.mail}
                    <span dir="ltr">${escapeHtml(m.email)}</span>
                  </div>
                  <button type="button" class="copy-contact-btn" data-copy="${escapeHtml(m.email)}" data-label="البريد" title="نسخ">
                    ${ICONS.copy}
                  </button>
                </div>
              ` : ''}
              ${!m.phone && !m.email ? `<span style="font-size:11.5px;color:var(--text-dim);">—</span>` : ''}
            </div>
          </td>
          <td style="padding:14px 12px;">
    ...
    tbody.querySelectorAll('tr[data-id]').forEach(tr => {
      tr.addEventListener('click', (e) => {
        if (e.target.closest('.row-btn') || e.target.closest('.copy-contact-btn')) return;
        openMemberModal(tr.dataset.id);
      });
      tr.addEventListener('mouseenter', () => { tr.style.background = 'rgba(249, 115, 22, 0.04)'; });
    ...
    // Copy buttons
    tbody.querySelectorAll('.copy-contact-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        copyToClipboard(btn.dataset.copy, btn.dataset.label || 'النص');
      });
    });
  }
  ...
  function renderPagination() {
    ...
    const btnStyle = (active, disabled) => `
      ...
      transition:all 0.2s;
      background:${active ? 'linear-gradient(135deg, #f97316, var(--accent))' : 'rgba(255,255,255,0.04)'};
      border:1px solid ${active ? 'transparent' : 'rgba(249, 115, 22, 0.2)'};
  ...
  async function openMemberModal(memberId) {
    ...
    try {
      ...
      body.innerHTML = `
        <div style="animation:fadeUp 0.3s;">
          <div style="display:flex;align-items:center;gap:14px;margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid var(--border-soft);">
            <div style="width:64px;height:64px;border-radius:16px;background:linear-gradient(135deg, rgba(249,115,22,0.15), rgba(var(--accent-rgb),0.15));border:1px solid rgba(249,115,22,0.3);display:flex;align-items:center;justify-content:center;font-family:'JetBrains Mono',monospace;font-weight:700;font-size:20px;color:#f97316;flex-shrink:0;">
          ...
          <h4 style="font-family:'Tajawal',sans-serif;font-size:14px;font-weight:700;color:#f97316;margin-bottom:12px;display:flex;align-items:center;gap:8px;">
            بيانات التواصل
          </h4>
          ...
          <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:24px;">
            ...
            ${m.phone ? `
              <div style="display:flex;align-items:center;gap:12px;padding:14px 16px;background:rgba(249,115,22,0.05);border:1px solid rgba(249,115,22,0.2);border-radius:12px;">
                ...
                <div style="flex:1;min-width:0;">
                  <div style="font-size:11.5px;color:var(--text-dim);margin-bottom:2px;">الموبايل</div>
                  <div style="font-size:14px;font-weight:700;color:var(--text);font-family:'JetBrains Mono',monospace;direction:ltr;text-align:right;">
                    ${escapeHtml(m.phone)}
                  </div>
                </div>
                <button type="button" class="copy-contact-btn" style="width:32px;height:32px;" onclick="copyFromModal('${escapeHtml(m.phone)}', 'الموبايل')">
                  ${ICONS.copy}
                </button>
              </div>
            ` : ''}
            ...
            ${m.email ? `
              <div style="display:flex;align-items:center;gap:12px;padding:14px 16px;background:rgba(139,92,246,0.05);border:1px solid rgba(139,92,246,0.2);border-radius:12px;">
                <div style="width:36px;height:36px;border-radius:8px;background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#8b5cf6;">
                  ${ICONS.mail}
                </div>
                <div style="flex:1;min-width:0;">
                  <div style="font-size:11.5px;color:var(--text-dim);margin-bottom:2px;">البريد الإلكتروني</div>
                  <div style="font-size:14px;font-weight:700;color:var(--text);font-family:'JetBrains Mono',monospace;direction:ltr;text-align:right;word-break:break-all;">
                    ${escapeHtml(m.email)}
                  </div>
                </div>
                <button type="button" class="copy-contact-btn" style="width:32px;height:32px;" onclick="copyFromModal('${escapeHtml(m.email)}', 'البريد')">
                  ${ICONS.copy}
                </button>
              </div>
            ` : ''}
            ...
            ${m.address ? `
              <div style="display:flex;align-items:center;gap:12px;padding:14px 16px;background:rgba(var(--accent-rgb),0.05);border:1px solid rgba(var(--accent-rgb),0.2);border-radius:12px;">
                <div style="width:36px;height:36px;border-radius:8px;background:rgba(var(--accent-rgb),0.1);border:1px solid rgba(var(--accent-rgb),0.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="width:18px;height:18px;stroke:var(--accent);fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;">
                    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
                    <circle cx="12" cy="10" r="3" />
                  </svg>
                </div>
                <div style="flex:1;min-width:0;">
                  <div style="font-size:11.5px;color:var(--text-dim);margin-bottom:2px;">العنوان</div>
      ...
      console.error('[PublicRelations] Detail error:', err);
      body.innerHTML = `
  ...
  window.copyFromModal = function (text, label) {
  ...
  function exportCSV() {
    ...
    const headers = [
      ...
      'البريد الإلكتروني', 'الشعبة', 'المحافظة', 'العنوان'
    ];
    ...
    link.href = url;
    link.download = `public_relations_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  function setupListeners() {
    ...
    if (contactFilter) {
      contactFilter.addEventListener('change', (e) => {
        filters.contact = e.target.value;
        currentPage = 1;
        loadMembers();
      });
    }
  ...
  function init() {
    ...
    window.onSupabaseReady(async (c) => {
      ...
      console.log('[PublicRelations] Ready. Role:', userRole);
    });
  }
