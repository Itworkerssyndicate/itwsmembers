...
(function () {
  ...
  async function checkAuth() {
    try {
      ...
      const { data: userData } = await client
        .from('users')
        .select('role, full_name, governorate_id')
        .eq('id', currentUser.id)
        .maybeSingle();

      userRole = userData?.role || 'committee';

      const allowedRoles = ['governorate_head', 'governorate_board', 'head', 'vice_president', 'deputy'];
      if (!allowedRoles.includes(userRole)) {
        alert('هذه الصفحة مخصصة لنقيب/مجلس المحافظة فقط');
      ...
      if (userData?.governorate_id) {
        ...
        try {
          const { data: govData } = await client
            .from('governorates')
            .select('name')
            .eq('id', userData.governorate_id)
            .maybeSingle();
          if (govData?.name) userGovernorate = govData.name;
        } catch (e) {}
      }
      ...
      window.currentUserGovernorate = userGovernorate;
      ...
      // Gov name
      const govNameEl = document.getElementById('govName');
      if (govNameEl) govNameEl.textContent = userGovernorate || 'كل المحافظات';
      ...
      console.error('[Governorate] Auth error:', err);
      window.location.href = 'login.html';
      return false;
    }
  }
  ...
  async function loadLookups() {
    try {
      const [brRes, typesRes] = await Promise.all([
      ...
      console.error('[Governorate] Lookups error:', err);
    }
  }

  /* ============================================
     BASE QUERY (governorate filter)
     ============================================ */
  ...
  async function loadStats() {
    try {
      ...
      console.error('[Governorate] Stats error:', e);
    }
  }
  ...
  async function loadReport() {
    ...
    try {
      ...
      const counts = {};
      data.forEach(m => {
        const key = m.branch_id || 'none';
        counts[key] = (counts[key] || 0) + 1;
      });

      reportData = Object.entries(counts).map(([bid, count]) => ({
        branch_id: bid === 'none' ? null : parseInt(bid),
        name: bid === 'none' ? 'بدون شعبة' : getBranchName(bid),
        count
      ...
      console.error('[Governorate] Report error:', e);
    }
  }
  ...
  async function loadMembers() {
    ...
    try {
      ...
      console.error('[Governorate] Load error:', err);
      if (tbody) {
        tbody.innerHTML = `
          <tr>
  ...
  async function openMemberModal(memberId) {
    ...
    try {
      ...
      console.error('[Governorate] Detail error:', err);
      body.innerHTML = `
  ...
  function exportCSV() {
    ...
    link.href = url;
    link.download = `governorate_${userGovernorate || 'all'}_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  function init() {
    ...
    window.onSupabaseReady(async (c) => {
      ...
      console.log('[Governorate] Ready. Role:', userRole, '| Gov:', userGovernorate);
    });
  }
